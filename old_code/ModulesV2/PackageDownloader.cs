...
namespace Orcus.Server.Service.ModulesV2
{
    public class PackageLoader
    {
        public static async Task<Dictionary<PackageIdentity, PackagePreFetcherResult>> GetPackagesAsync(
            IEnumerable<ResolvedAction> actions, IModuleApplicationService appService, PackageDownloadContext downloadContext, ILogger logger,
            ...
            foreach (var action in actions)
            {
                if (action.Action == ResolvedActionType.Install && seen.Add(action.PackageIdentity))
                {
                    ...
                    if (!uninstalledPackages.Contains(action.PackageIdentity))
                    {
                        ...
                        localFile = appService.GetLocalPackagePath(action.PackageIdentity);
