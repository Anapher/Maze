...
namespace x264net
{
	...
	array<X264Nal^>^ X264Net::EncodeFrame(void* rgb_data)
	{
		// increment presentation timestamp; just because.
		pic_in->i_pts = frame++;

		// Convert RGB in pinned_rgb_data to YUV420P (a.k.a. YUV420 / I420) in pic_in
		Bitmap2Yuv420p_calc2(pic_in->img.plane[0], (uint8_t*) rgb_data, Width, Height);
		...
		if (frame_size >= 0)
		{
			array<X264Nal^>^ managed_NAL_array = gcnew array<X264Nal^>(i_nals);

			for (int i = 0; i < i_nals; i++)
			{
				X264Nal^ nal = gcnew X264Nal();
				nal->Pointer = nals[i].p_payload;
				nal->Length = nals[i].i_payload;

				managed_NAL_array[i] = nal;
			}
