    ...
    public abstract class TextOutputFormatter : OutputFormatter
    {
        ...
        public virtual Encoding SelectCharacterEncoding(OutputFormatterWriteContext context)
        {
            ...
            if (SupportedEncodings.Count == 0)
            {
                throw new InvalidOperationException(
                    $"The list of '{nameof(SupportedEncodings)}' must not be empty. Add at least one supported encoding.");
            }
        ...
        public override Task WriteAsync(OutputFormatterWriteContext context)
        {
            ...
            if (!selectedMediaType.HasValue)
            {
                ...
                {
                    throw new InvalidOperationException(
                        $"No supported media type registered for output formatter '{GetType().FullName}'. There must be at least one supported media type registered in order for the output formatter to write content.");
                }
            }
        ...
        {
            throw new InvalidOperationException(
                $"'{nameof(WriteResponseBodyAsync)}({nameof(OutputFormatterWriteContext)})' is not supported by '{nameof(TextOutputFormatter)}'. Use '{nameof(WriteResponseBodyAsync)}({nameof(OutputFormatterWriteContext)},{nameof(Encoding)})' instead.");
        }
