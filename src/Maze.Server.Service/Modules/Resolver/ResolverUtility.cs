    ...
    public static class ResolverUtility
    {
            ...
            IEnumerable<PackageSource> packageSources)
        {
                ...
                !dependantPackages.Any())
            {
                ...
                {
                    message.AppendFormat(CultureInfo.CurrentCulture, "Unable to resolve dependency '{0}'. Source(s) used: {1}.", problemPackageId, packageSourceList);
                }
                else
                {
                    message.AppendFormat(CultureInfo.CurrentCulture, "Unable to resolve dependency '{0}'.", problemPackageId, packageSourceList);
                }
            }
            else
            {
                ...
                if (packageOptions.Count() == 1 && (configEntry == null || !configEntry.HasAllowedVersions))
                {
                    ...
                    message.AppendFormat(CultureInfo.CurrentCulture, "Unable to resolve dependencies. '{0}' is not compatible with {1}.", problemPackageString, String.Join(", ", dependantPackages));
                }
                else
                {
                    // Return an error with the problem package id, and all parent packages that might have caused the issue
                    message.AppendFormat(CultureInfo.CurrentCulture, "Unable to find a version of '{0}' that is compatible with {1}.", problemPackageId, String.Join(", ", dependantPackages));
                }
            }
