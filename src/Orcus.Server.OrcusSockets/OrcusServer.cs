...
namespace Orcus.Server.OrcusSockets
{
    public class OrcusServer
    {
        ...
        private readonly ILogger<OrcusServer> _logger;
        ...
        public OrcusServer(OrcusSocket socket, ILogger<OrcusServer> logger)
        {
            ...
            if (logger.IsEnabled(LogLevel.Trace))
                socket.DataReceivedEventArgs += (sender, args) =>
                    _logger.LogTrace($"Data received with Message Opcode {args.Opcode}");
        }
        ...
        private void ProcessMessage(ArraySegment<byte> buffer)
        {
            ...
            {
                _logger.LogError("Received message for channel {channelId} which does not exist.", channelId);
        ...
        {
            _logger.LogDebug("Request received (isCompleted = {isCompleted}), length = {length}",
            ...
            else
            {
                ...
                {
                    _logger.LogDebug("Added request {requestId} to active requests", requestId);
                }
                else
                {
                    _logger.LogError(
        ...
        {
            _logger.LogDebug("Response received (isCompleted = {isCompleted}), total length = {length}",
                isCompleted, buffer.Count);
            ...
            else
            {
                ...
                {
                    _logger.LogDebug("Added response {requestId} to active responses", requestId);
                }
                else
                {
                    _logger.LogError(
            ...
            {
                _logger.LogError("No TaskCompletionSource for request {requestId} found.", requestId);
                response.Dispose();
            }
